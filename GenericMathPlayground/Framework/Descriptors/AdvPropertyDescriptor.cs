    ...
    : PropertyDescriptor, IEquatable<AdvPropertyDescriptor>
{
    ...
    public AdvPropertyDescriptor(FieldInfo field)
        : this(field?.Name ?? string.Empty, field)
    { }
    ...
    public AdvPropertyDescriptor(string name, FieldInfo? field)
        : base(name, (Attribute[])(field?.GetCustomAttributes(typeof(Attribute), true) ?? Array.Empty<Attribute>()))
    {
        info = field;
        this.field = field;
    ...
    public AdvPropertyDescriptor(PropertyInfo property)
        : this(property?.Name ?? string.Empty, property)
    { }
    ...
    public AdvPropertyDescriptor(string name, PropertyInfo? property)
        : base(name, (Attribute[])(property?.GetCustomAttributes(typeof(Attribute), true) ?? Array.Empty<Attribute>()))
    {
        info = property;
    ...
    /// </summary>
    public override Type ComponentType => info?.DeclaringType ?? typeof(object);

    /// <summary>
    /// Gets the property type.
    /// </summary>
    public override Type PropertyType => (field is null ? property?.PropertyType : field.FieldType) ?? typeof(object);
    ...
    /// <returns>The <see cref="bool"/>.</returns>
    public bool Equals(AdvPropertyDescriptor? other) => info?.Equals(other?.info) ?? false;
    #endregion Methods
}
