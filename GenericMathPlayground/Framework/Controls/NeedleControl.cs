    ...
    : UserControl
{
    ...
    private void NeedleControl_MouseMove(object sender, MouseEventArgs e)
    {
        ...
        if (selecting)
        {
            Angle = Operations.Angle<double, double>(e.X, e.Y, center.X, center.Y);
            Invalidate(true);
        }
    }
    ...
    private static void DrawTicks(Graphics g, RectangleF rect)
    {
        ...
        for (float i = 0f; i < 2f * MathF.PI; i += Operations.DegreesToRadians<float, float>(15f))
        {
            (var sin, var cos) = MathF.SinCos(i);
            var pointInside = new PointF(center.X + (radiusInside * cos), center.Y + (radiusInside * sin));
            var pointOutside = new PointF(center.X + (radiusOutside * cos), center.Y + (radiusOutside * sin));
    ...
    private static void DrawNeedle(Graphics g, RectangleF rect, double angle)
    {
        ...
        (var sin, var cos) = MathF.SinCos((float)angle);
        var point = new PointF(center.X + (radius * cos), center.Y + (radius * sin));
    ...
    public static PointF Center(RectangleF rectangle) => new(rectangle.Left + ((rectangle.Right - rectangle.Left) * 0.5f), rectangle.Top + ((rectangle.Bottom - rectangle.Top) * 0.5f));
    ...
    /// <remarks><para>Be sure to cache the results of this method if used repeatedly, as it is recalculated each time.</para></remarks>
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public static PointF Center(Size size) => new(size.Width * 0.5f, size.Height * 0.5f);
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public static Size Inflate(Size size, int amount) => new(size.Width + amount, size.Height + amount);
}
