...
public static partial class Operations
{
    ...
    /// <returns></returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public static ValueMatrix2x2<T> PermuteMatrix<T>(ValueMatrix2x2<T> u, ValueVector3<T> M)
        where T : INumber<T>
    {
        var result = ValueMatrix2x2<T>.MultiplicativeIdentity;
        if (M.I * M.K < M.J * M.J && u.Determinant * (M.I - M.K) < T.Zero)
        {
            (result.ColumnY, result.ColumnX) = (result.ColumnX, result.ColumnY);
        }
        return result;
    }
    #endregion
